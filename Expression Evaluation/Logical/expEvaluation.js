...
window.view = {
	...
	expressionValidation: function(expression) {
		var expressionValidationKey = expression.match(/([e-z]|((\+|\-|\*|\/)\s){2,9}|((\&|\|){3,9})|(\&\|)|(\|\&))/g);
	...
	addChildReasoning: function() {
		...
		case 0:
		  ...
		  break;
		case 1:
		  	childDiv.textContent = '&& operater has higher preference over || operator and then following left to right rule.'
		  break;
		default:
		 	childDiv.textContent = '|| operater has higher preference over remaining Logical operators and then following left to right rule.'
		}
