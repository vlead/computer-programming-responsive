...
window.view = {
	...
	expressionValidation: function(expression) {
		var expressionValidationKey = expression.match(/([e-z]|((\+|\-|\*|\/)\s){3,9})/g);
	...
	getExpression: function() {
		...
		var expressionwithoutAnyGap = this.removeGapBetweenDigitAndOperators(properExpression)
	...
	addChildReasoning: function() {
		...
		case 0:
		  ...
		  break;
		case 1:
		  	childDiv.textContent = '% operater has higher preference according to the precedence order of remaining operators and then following left to right rule.'
		  break;
		case 2:
			childDiv.textContent = '/ operater has higher preference according to the precedence order of remaining operators and then following left to right rule.'
		  break;
	...
	expressionEvaluationOfBrackets: function() {	
		...
		else
		{
			...
			expression =  this.expressionEvaluationOfModulation()
		}
		return expression;
	},
	expressionEvaluationOfModulation: function() {	
		var expression = document.getElementById("childOfExpressionDivision"+this.childDivNo).textContent;	
		...
		{
			this.functionPrecedence = this.functionPrecedence + 1;
			expression =  this.expressionEvaluationOfDividation()
		}
		return expression;
	},
	expressionEvaluationOfDividation: function() {	
		var expression = document.getElementById("childOfExpressionDivision"+this.childDivNo).textContent;	
		var matches = expression.match(/(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)\s\/\s(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)/);
		if(matches!=null)
		{
		...
		{
			this.functionPrecedence = this.functionPrecedence + 1;
			expression =  this.expressionEvaluationOfMultiplication()
		}
		return expression;
	},
	expressionEvaluationOfMultiplication: function() {	
		var expression = document.getElementById("childOfExpressionDivision"+this.childDivNo).textContent;	
		var matches = expression.match(/(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)\s\*\s(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)/);
		if(matches!=null)
		{
		...
		{
			this.functionPrecedence = this.functionPrecedence + 1;
			expression =  this.expressionEvaluationOfSubtraction()
		}
		return expression;
	},
	expressionEvaluationOfSubtraction: function() {	
		var expression = document.getElementById("childOfExpressionDivision"+this.childDivNo).textContent;	
		var matches = expression.match(/(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)\s\-\s(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)/);
		if(matches!=null) {
			expression = this.getNextEvaluationStep(matches[0],expression)
		...
		{
			this.functionPrecedence = this.functionPrecedence + 1;
			expression =  this.expressionEvaluationOfAddition()
		}
		return expression;
	},
	expressionEvaluationOfAddition: function() {	
		var expression = document.getElementById("childOfExpressionDivision"+this.childDivNo).textContent;	
		var matches = expression.match(/(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)\s\+\s(\-\d+\.\d+|\+\d+\.\d+|\d+\.\d+|\.\d+|\-\d+|\+\d+|\d+)/);
		if(matches!=null) {
			expression = this.getNextEvaluationStep(matches[0],expression)
	...
	expressionEvaluation: function() {	
		...
		case 0:
		  ...
		  break;
		case 1:
		  expressionResult =  this.expressionEvaluationOfModulation();
		  break;
		case 2:
		  expressionResult =  this.expressionEvaluationOfDividation();
		  break;
		case 3:
		  expressionResult =  this.expressionEvaluationOfMultiplication();
		  break;
		case 4:
		  expressionResult =  this.expressionEvaluationOfSubtraction();
		  break;
		default:
		  expressionResult =  this.expressionEvaluationOfAddition();
		}
